...
def generate_content(api_token: str, taks_id: str) -> str:
    url = BASE_URL + '/api/ppt/v2/generateContent'
    body = json.dumps({
        'id': taks_id,
        "stream": True,
        "length": "medium",
...
def create_task(api_token,type_,content,file=None):
    ...
    response = post_json(url, {'token': api_token}, body)
    if response.status_code != 200:
        raise RuntimeError('生成PPT任务id，httpStatus=' + str(response.status_code))
    result = json.loads(response.text)
    if result['code'] != 0:
        raise RuntimeError('生成PPT异常：' + result['message'])
